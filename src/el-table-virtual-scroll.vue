...
export default {
  ...
  methods: {
    ...
    handleScroll (shouldUpdate = true) {
      ...
      //横向滚动不需要处理事件
      if (typeof shouldUpdate === 'object' && shouldUpdate.type === 'scroll' && this.scroller.scrollTop === this.scrollPos[0]) {
        return
      }
      //解决scroll事件死循环的问题
      if (typeof shouldUpdate === 'boolean' && shouldUpdate === false) return
